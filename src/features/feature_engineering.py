...
def extract_features_and_targets(
    ...
    y = df["fraud_label"].copy() if "fraud_label" in df.columns else pd.Series(np.zeros(len(df), dtype=int))
    attack_types = df["attack_type"].copy() if "attack_type" in df.columns else pd.Series(["unknown"] * len(df))
