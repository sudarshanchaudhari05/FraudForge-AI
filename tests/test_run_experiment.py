...
def test_cli_argument_parsing():
    ...
    with patch("sys.argv", ["run_experiment.py", "--samples", "5000", "--fraud-ratio", "0.25", "--mutation-intensity", "0.80", "--zero-day"]):
        ...
        args = parser.parse_args(["--samples", "5000", "--fraud-ratio", "0.25", "--mutation-intensity", "0.80", "--zero-day"])

        assert args.samples == 5000
        assert args.fraud_ratio == 0.25
        assert args.mutation_intensity == 0.80
        assert args.zero_day is True


def test_run_zero_day_experiment_orchestration(tmp_path):
    """Verify run_zero_day_experiment runs successfully."""
    from run_experiment import run_zero_day_experiment
    ...
    assert "generalization_test_results" in results
    assert "normal_test_regression_metrics" in results
    assert "novel_training_attacks_results" in results
