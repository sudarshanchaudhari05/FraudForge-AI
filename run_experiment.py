...
def run_zero_day_experiment(
    ...
    """Execute the 8-stage Zero-Day Discovery -> Hardening -> Re-Attack experiment."""
    from src.adversarial.zero_day_hardening import ZeroDayHardeningPipeline

    print("=" * 60)
    print("        FRAUDFORGE AI -- ZERO-DAY ADAPTIVE DEFENSE")
    print("=" * 60)
    print()

    def stage_progress_callback(stage_num: int, stage_desc: str) -> None:
        print(f"[{stage_num}/8] {stage_desc}")
        print("      [OK] Ready\n")
    ...
    # Print Zero-Day summary table
    gen_res = results["generalization_test_results"]
    reg_res = results["normal_test_regression_metrics"]
    ...
    print("                    BEFORE -> AFTER")
    print("=" * 60)
    print(f"\n{'Metric':<30} {'Baseline':<14} {'Hardened':<14}")
    print("-" * 60)
    print(f"{'Unseen Gen-2 Attack Recall':<30} {gen_res['overall_baseline_detection_rate']:>5.1f}%{'':<8} {gen_res['overall_hardened_detection_rate']:>5.1f}%")
    print(f"{'Unseen Gen-2 Missed Attacks':<30} {gen_res['baseline_misses']:<14d} {gen_res['hardened_misses']:<14d}")
    print(f"{'Generalization Gain':<30} +{gen_res['generalization_gain_percentage_points']:.2f} percentage points")
    print(f"{'Miss Reduction Rate':<30} {gen_res['miss_reduction_percentage']:.1f}%")
    print(f"{'Normal FPR Cost':<30} {base_norm['false_positive_rate']*100:>5.2f}%{'':<8} {hard_norm['false_positive_rate']*100:>5.2f}% ({reg_res['fpr_cost']:+.2f}%)")
    print("\n" + "=" * 60)
    print("      RED TEAM -> BLUE TEAM LOOP COMPLETE")
    print("=" * 60)
    print("\nIMPORTANT: These are synthetic defensive attack scenarios.")
    print("They are NOT real-world zero-day vulnerabilities.\n")
...
def main() -> None:
    ...
    parser.add_argument(
        "--zero-day",
        action="store_true",
        help="Execute the full Phase 6 Zero-Day Discovery & Hardening loop",
    )
    parser.add_argument(
        "--risk-policy",
        action="store_true",
        help="Execute Phase 7 Risk-Aware Decision Engine & Policy Evaluation",
    )
    ...
    try:
        if args.zero_day:
            results = run_zero_day_experiment(
                samples=args.samples,
                fraud_ratio=args.fraud_ratio,
                mutation_intensity=args.mutation_intensity,
            )
            print(f"[+] Zero-Day summary saved to: {EXPERIMENTS_DIR / 'zero_day_hardening_summary.txt'}")
        else:
        ...
        # Optional Phase 7 Risk Policy Evaluation
        if args.risk_policy:
            from src.detection.threshold_analysis import ThresholdAnalysisEngine
            print("\n" + "=" * 60)
            print("  PHASE 7 -- RISK-AWARE DECISION ENGINE EVALUATION")
            print("=" * 60)
            thresh_engine = ThresholdAnalysisEngine(output_dir=EXPERIMENTS_DIR)
            thresh_engine.run_full_evaluation()
